...
public static class PhotonNetwork
{
    ...
    public const string versionPUN = "1.9.6";
    ...
    public static void Disconnect()
    {
        ...
        if(room!=null)
            LeaveRoom();//TODO: temporary workaround for clients getting stuk in a room while being disconnected
    ...
    public static RoomInfo[] GetRoomList()
    {
        ...
        RoomInfo[] array = new RoomInfo[networkingPeer.mGameList.Count];
    ...
    /// <param name="prefab">The prefab to instantiate (it's name must be found in Resources).</param>
    ...
    {
        return Instantiate(prefab.name, position, rotation, group, null);
    }
    ...
    {
        return Instantiate(prefab.name, position, rotation, group, data);
    }
    ...
    public static void Destroy(int viewID)
    {
        ...
        if (view.isMine || player.isMasterClient)
        {
            Destroy(view);
        }
        else
        {
            Debug.LogError("Destroy: Could not destroy view ID [" + view + "]. As it is not ours!");
        }
    }
